...
var selectedGame =
   getParameterByName("game", window.location) != null
      ? getParameterByName("game", window.location)
      : "zelda3";
...
var metroidMode =
   getParameterByName("metroidMode", window.location) != null
      ? getParameterByName("metroidMode", window.location)
      : "";
...
function build_img_url(item, useGame = selectedGame) {
   ...
   if (item.indexOf("boss") == -1 && item.indexOf("chest") == -1) {
      if (item == "bomb") {
         ...
         metroid3items.indexOf(item) > -1 ||
         metroid3items.indexOf(item.substr(0, item.length - 1)) > -1
      ) {
         useGame = "metroid3";
      } else if (
         zelda3items.indexOf(item) > -1 ||
         zelda3items.indexOf(item.substr(0, item.length - 1)) > -1
      ) {
         useGame = "zelda3";
      }
   }
