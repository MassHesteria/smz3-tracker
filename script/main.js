...
for (var i = 0; i < chests[selectedGame].length; i++) {
   ...
   chests[selectedGame][i].isSpicy =
      ["metroid3"].indexOf(selectedGame) > -1 && spicyChests.indexOf(i) > -1;
}
...
function setConfigObject(configobj) {
   ...
   document.getElementsByName("showmap")[0].checked =
      !!configobj[selectedGame].map; // Map Enabled?
   document.getElementsByName("showmap")[0].onchange();
   document.getElementsByName("itemdivsize")[0].value =
      configobj[selectedGame].iZoom; // Inventory Scale
   document.getElementsByName("itemdivsize")[0].onchange();
   document.getElementsByName("mapdivsize")[0].value =
      configobj[selectedGame].mZoom; // Map Scale (Z3 only)
   document.getElementsByName("mapdivsize")[0].onchange();

   document
      .getElementsByName("maporientation")
      [configobj[selectedGame].mOrien].click(); // Map Orientation (Horizontal, Vertical) (Z3 only)

   var mappositions = ["Above", "Below", "Side"];
   document
      .getElementsByName("mapposition")
      [mappositions.indexOf(configobj[selectedGame].mPos)].click(); // Map Position (Above, Below, Side) (Z3 only)

   if (["zelda3"].indexOf(selectedGame) > -1) {
      var mapstates = ["standard", "open", "inverted", "retro"];
      document
         .getElementsByName("mapstate")
         [mapstates.indexOf(configobj[selectedGame].mapState)].click(); // Map State (Standard, Open, Inverted, Retro) (Z3 only)
   }

   document.getElementsByName("swordless")[0].checked =
      !configobj[selectedGame].mapSwords; // Swordless? (Z3 only)
   document.getElementsByName("swordless")[0].onchange();
   document.getElementsByName("ohko")[0].checked =
      !!configobj[selectedGame].mapOHKO; // OHKO? (Z3 only)
   document.getElementsByName("ohko")[0].onchange();

   if (["zelda3"].indexOf(selectedGame) > -1) {
      var itemplacements = ["basic", "advanced"];
      document
         .getElementsByName("itemplacement")
         [
            itemplacements.indexOf(configobj[selectedGame].itemPlacement)
         ].click(); // Item Placement (Basic, Advanced) (Z3 only)
   }
   ...
   document
      .getElementsByName("maplogic")
      [maplogics.indexOf(configobj[selectedGame].mapLogic)].click(); // Map Logic

   if (["metroid3"].indexOf(selectedGame) > -1) {
      var chestskins = ["lights", "nolights", "nothing"];
      document
         .getElementsByName("chestskin")
         [chestskins.indexOf(configobj[selectedGame].chestSkin)].click(); // Chest Skin (Lights, No Lights, Nothing) (M3 only)
   }

   document.getElementsByName("showchest")[0].checked =
      !!configobj[selectedGame].showChests; // Show Chests on Dungeon squares (Z3 only)
   document.getElementsByName("showchest")[0].onchange();
   document.getElementsByName("showcrystal")[0].checked =
      !!configobj[selectedGame].showPrizes; // Show Prizes on Dungeon squares (Z3 only)
   document.getElementsByName("showcrystal")[0].onchange();
   document.getElementsByName("showmedallion")[0].checked =
      !!configobj[selectedGame].showMedals; // Show Medallions on Dungeon squares (Z3 only)
   document.getElementsByName("showmedallion")[0].onchange();
   document.getElementsByName("showlabel")[0].checked =
      !!configobj[selectedGame].showLabels; // Show Labels on Dungeon/Boss squares
...
function updateConfigFromFirebase(configobj) {
   ...
   if (
      !existingConfig ||
      !existingConfig.ts ||
      existingConfig.ts < configobj.ts
   ) {
...
function getConfigObject() {
   ...
   configobj[selectedGame].map =
      document.getElementsByName("showmap")[0].checked; // Map Enabled?
   configobj[selectedGame].iZoom =
      document.getElementsByName("itemdivsize")[0].value; // Inventory Scale
   configobj[selectedGame].mZoom =
      document.getElementsByName("mapdivsize")[0].value; // Map Scale (Z3 only)

   configobj[selectedGame].mOrien = document.getElementsByName(
      "maporientation"
   )[1].checked
      ? 1
      : 0; // Map Orientation (Horizontal, Vertical) (Z3 only)
   configobj[selectedGame].mPos = document.querySelector(
      'input[name="mapposition"]:checked'
   ).value; // Map Position (Above, Below, Side) (Z3 only)
   configobj[selectedGame].mapLogic = document.querySelector(
      'input[name="maplogic"]:checked'
   ).value; // Map Logic
   configobj[selectedGame].mapOHKO =
      document.getElementsByName("ohko")[0].checked; // OHKO? (Z3 only)
   if (["zelda3"].indexOf(selectedGame) > -1) {
      configobj[selectedGame].mapSwords =
         !document.getElementsByName("swordless")[0].checked; // Swords? (Z3 only)
   } else if (["metroid3"].indexOf(selectedGame) > -1) {
      configobj[selectedGame].chestSkin = document.querySelector(
         'input[name="chestskin"]:checked'
      ).value; // Chest Skin (Lights, No Lights, Nothing) (M3 only)
   }

   configobj[selectedGame].mapState = document.querySelector(
      'input[name="mapstate"]:checked'
   ).value; // Map State (Standard, Open) (Z3 only)
   configobj[selectedGame].showChests =
      document.getElementsByName("showchest")[0].checked; // Show Chests on Dungeon squares (Z3 only)
   configobj[selectedGame].showPrizes =
      document.getElementsByName("showcrystal")[0].checked; // Show Prizes on Dungeon squares (Z3 only)
   configobj[selectedGame].showMedals =
      document.getElementsByName("showmedallion")[0].checked; // Show Medallions on Dungeon squares (Z3 only)
   configobj[selectedGame].showLabels =
      document.getElementsByName("showlabel")[0].checked; // Show Labels on Dungeon/Boss squares
...
selectGame +=
   '<a href="faq.html">FAQ</a> | <a href="http://github.com/masshesteria/smz3-tracker/">GitHub</a>';
...
function highlight(x) {
   document.getElementById(x).style.backgroundImage =
      "url(" + build_img_url("highlighted") + ")";
   document.getElementById("caption").innerHTML =
      selectGame + " | " + chests[selectedGame][x].titleEquipment + " ]";
}

function unhighlight(x) {
   document.getElementById(x).style.backgroundImage =
      "url(" + build_img_url("poi") + ")";
...
function highlightDungeon(x) {
   document.getElementById("dungeon" + x).style.backgroundImage =
      "url(" + build_img_url("highlighted") + ")";
   document.getElementById("caption").innerHTML =
      selectGame + " | " + dungeons[selectedGame][x].titleEquipment + " ]";
}
...
function setZoom(target, sender) {
   ...
   if (["metroid3"].indexOf(selectedGame) > -1) {
      ...
      if (target == "mapdiv" && metroidMode == "accessibility") {
         document.getElementById("itemdiv").style.top =
            (660 * sender.value) / 100;
         setZoom("itemdiv", {
            value: Math.round(sender.value * 1.08),
         });
      }
   }
...
function showSettings(sender) {
   if (trackerData[selectedGame].editMode) {
      trackerData[selectedGame].showChests =
         document.getElementsByName("showchest")[0].checked;
      trackerData[selectedGame].showPrizes =
         document.getElementsByName("showcrystal")[0].checked;
      trackerData[selectedGame].showMedals =
         document.getElementsByName("showmedallion")[0].checked;
      trackerData[selectedGame].showLabels =
         document.getElementsByName("showlabel")[0].checked;
...
function refreshMapMedallion(d) {
   ...
   if (dungeons[selectedGame][d]) {
      if (trackerData[selectedGame].dungeonbeaten[d])
         document.getElementById("bossMap" + d).className =
            "mapspan boss opened";
      else
         document.getElementById("bossMap" + d).className =
            "mapspan boss " +
            dungeons[selectedGame][d].isBeatable().getClassName();

      if (trackerData[selectedGame].dungeonchests[d] > 0)
         document.getElementById("dungeon" + d).className =
            "mapspan 1dungeon " +
            dungeons[selectedGame][d].canGetChest().getClassName();
      ...
      var dungeonName;
      if (d === 8) dungeonName = "Misery Mire";
      else dungeonName = "Turtle Rock";
      dungeons[selectedGame][d].name =
         dungeonName +
         " " +
         mini("medallion" + trackerData[selectedGame].medallions[d]) +
         mini("lantern");
   }
}
   ...
   for (k = 0; k < chests[selectedGame].length; k++) {
      ...
      if (roomid == "smalttpr") {
         ...
         if (chest.name == "Link's House") {
            ...
            if (has("state.inverted")) {
               // Inverted, move to Light World
               chestDOM.classList.remove("bombshop");
               chestDOM.classList.add("lonkshoose");
            } else if (!has("state.inverted")) {
               // Not Inverted, move to Dark World
               chestDOM.classList.add("bombshop");
               chestDOM.classList.remove("lonkshoose");
            }
...
function refreshMap() {
   ...
   for (k = 0; k < dungeons[selectedGame].length; k++) {
      if (trackerData[selectedGame].dungeonbeaten[k]) {
         ...
         document.getElementById("bossMap" + k).className =
            "mapspan boss " +
            dungeons[selectedGame][k].isBeatable().getClassName();
      }

      if (trackerData[selectedGame].dungeonchests[k]) {
         document.getElementById("dungeon" + k).className =
            "mapspan dungeon " +
            dungeons[selectedGame][k].canGetChest().getClassName();
      } else {
         document.getElementById("dungeon" + k).className =
            "mapspan dungeon opened";
      }
...
function itemConfigClick(sender) {
   ...
   if (trackerData[selectedGame].selected.item) {
      document.getElementById(
         trackerData[selectedGame].selected.item
      ).style.border = "0px";
      ...
      itemGrid[selected.row][selected.col]["item"].style.border =
         "1px solid white";
...
function populateMapdiv(useGame = "zelda3") {
   ...
   for (k = 0; k < chests[useGame].length; k++) {
      ...
      if (chests[useGame][k]) {
         s.title =
            chests[useGame][k].titleStripped +
            (useGame == "metroid3" && typeof wikiRoomNames[k] != "undefined"
               ? "\n" + '"' + wikiRoomNames[k] + '"'
               : "");
      ...
      if (trackerData[useGame] && trackerData[useGame].chestsopened[k])
         ...
         s.className =
            "mapspan chest " + chests[useGame][k].isAvailable().getClassName();
   ...
   for (k = 0; k < dungeons[useGame].length; k++) {
      ...
      s.style.backgroundImage =
         "url(" + build_img_url("boss" + k + itemsMax["boss" + k]) + ")";
      s.id = "bossMap" + k;
      ...
      s.className =
         "mapspan boss " + dungeons[useGame][k].isBeatable().getClassName();
      ...
      s.style.top = dungeons[useGame][k].y;
      s.className =
         "mapspan dungeon " + dungeons[useGame][k].canGetChest().getClassName();
      if (dungeons[useGame][k].x == "" && dungeons[useGame][k].y == "") {
         s.style.display = "none";
      }
      mapdiv.appendChild(s);
...
function populateItemconfig() {
   ...
   for (var key in trackerData[selectedGame].items) {
      ...
      if (
         gameItems[thisGame].indexOf(key) > -1 ||
         gameItems[altGame].indexOf(key) > -1 ||
         key == "blank"
      ) {
         ...
         if (typeof trackerData[thisGame].items[key] === "boolean") {
            rowitem.style.backgroundImage =
               "url(" + build_img_url(key, useGame) + ")";
         } else if (
            key.indexOf("heart") === 0 ||
            key.indexOf("missile") > -1 ||
            key.indexOf("powerbomb") === 0 ||
            key.indexOf("tank") > -1
         ) {
            rowitem.style.backgroundImage =
               "url(" + build_img_url(key, useGame) + ")";
            ...
            rowitem.style.backgroundImage =
               "url(" + build_img_url(key + itemsMax[key], useGame) + ")";
         }
         if (
            key.indexOf("boss") === 0 &&
            dungeons[thisGame][key.substring(4)]
         ) {
            rowitem.style.backgroundImage =
               "url(" + build_img_url(key + itemsMax[key], useGame) + ")";
...
function initTracker() {
   ...
   if (["zelda3"].indexOf(selectedGame) > -1) {
      ...
      if (zeldaMode == "regions") {
         document
            .getElementsByName("maplogic")
            [logics.indexOf("minorGlitches")].click();
      } else {
         if (trackerData.zelda3.mapLogic == "minorGlitches") {
            document
               .getElementsByName("maplogic")
               [logics.indexOf("glitchless")].click();
         }
      ...
      for (var k in Object.keys(newValues.itemValues)) {
         ...
         if (k.indexOf("boss") == -1 && k.indexOf("chest") == -1) {
            for (var loadGameName in gameNames) {
               ...
               if (
                  trackerData[loadGameName] &&
                  trackerData[loadGameName].items
               ) {
                  if (k in trackerData[loadGameName].items) {
                     for (var setGameName in gameNames) {
                        setGameName = gameNames[setGameName];
                        if (
                           setGameName in trackerData &&
                           "items" in trackerData[setGameName]
                        ) {
                           if (k in trackerData[setGameName].items) {
                              trackerData[setGameName].items[k] =
                                 newValues.itemValues[k];
                           } else {
                              newValue = {
                                 k: newValues.itemValues[k],
                              };
                              trackerData[setGameName].items = extend(
                                 trackerData[setGameName].items,
                                 newValue
                              );
                           }
                        }
                     ...
                     trackerData[loadGameName].items = extend(
                        trackerData[loadGameName].items,
                        newValue
                     );
                  }
...
function updateAll() {
   if (
      ...
      trackerData[selectedGame].dungeonbeaten &&
      trackerData[selectedGame].prizes &&
      trackerData[selectedGame].medallions &&
...
function confirmSaveConfigToFirebase() {
   var confirm = window.confirm(
      "Do you want to push your configuration to all other users of your tracker? This will overwrite their settings. (Use this to get a remote browser to match how this browser appears.)"
   );
   if (confirm) {
...
Vue.component("tracker-table", {
   ...
   props: ["itemRows", "trackerData", "trackerData"],
   computed: {
...
Vue.component("tracker-cell", {
   ...
   props: ["itemValue", "itemName", "columnIndex", "rowIndex", "trackerData"],
   computed: {
      bossNum: function () {
      ...
      backgroundImage: function () {
         ...
         if (this.itemName === "blank") {
            return this.trackerData[selectedGame].editMode
               ? "url(" + build_img_url("blank") + ")"
               : "none";
         ...
         return (
            ...
            build_img_url(
               this.itemName +
                  (this.trackerData[selectedGame].editMode
      ...
      chestImage: function () {
         ...
         if (
            ...
            return (
               ...
               build_img_url(
                  "chest" +
                     this.trackerData[selectedGame].dungeonchests[this.bossNum]
               ) +
               ")"
            );
         }
      ...
      medallionImage: function () {
         ...
         if (
            ...
            this.trackerData[selectedGame] &&
            this.trackerData[selectedGame].showMedals
         ) {
            return (
               "url(" +
               build_img_url(
      ...
      itemClass: function () {
         ...
         if (
            itemGame != "averge1" &&
            gameItems[universe + "3"].indexOf(this.itemName) == -1
         ) {
      ...
      ohkoClass: function () {
         ...
         if (
            this.itemLabel.toLowerCase() == "tunic" &&
            this.trackerData[selectedGame].mapOHKO
         ) {
            return "ohko";
         }
         return null;
   ...
   methods: {
      clickCell: function (amt) {
         ...
         if (this.trackerData[selectedGame].editMode) {
            Vue.set(
               ...
               this.columnIndex,
               this.trackerData[selectedGame].selected.item || "blank"
            );
            return;
         }
         ...
         if (this.bossNum) {
            ...
            this.trackerData[selectedGame].dungeonbeaten[this.bossNum] =
               !this.trackerData[selectedGame].dungeonbeaten[this.bossNum];
            updateAll();
         }
      ...
      clickMedallion: function (amt) {
         ...
         this.trackerData[selectedGame].medallions.splice(
            this.bossNum,
            1,
            newMedallion
         );
      ...
      clickPrize: function (amt) {
         var newPrize =
            (this.trackerData[selectedGame].prizes[this.bossNum] + amt + 5) % 5;
         // need to use splice here instead of just setting it the normal way or vue won't pick up the change
         this.trackerData[selectedGame].prizes.splice(
            this.bossNum,
            1,
            newPrize
         );
